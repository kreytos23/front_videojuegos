...
const peticionAPI=async()=>{
  await axios.get('https://backend-games-mysql.herokuapp.com/getgame/platform') 
  .then(response=>{
...
await axios.get('https://backend-games-mysql.herokuapp.com/getGame') 
.then(response=>{
  var respuesta1=response.data;
  var auxfecha=[], auxcantidad1=[];
...
PlataformaD.map(item => {
    for (let index = 0; index < PlataformaD2.length; index++) {
        if(item.Consola == PlataformaD2[index].Platform){
            item.Cuantity == item.Cuantity + PlataformaD2[index].Cuantity;
        }
    }
})
        ...
        <div className="row">
          <div className="col-7">
            ...
            <Juegosporaño GAMESFORYEAR={GAMESFORYEAR}/>
             ...
             <Listad PlataformaD={PlataformaD}/>
          </div>
          <div className="col-5"  >
